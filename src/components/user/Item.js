...
const Item = (props) => {
    ...
    return (
        <View style={styles.container} >
            ...
            <Icon name={props.icon} size={props.size} style={[styles.ico, props.style]} />
            <Text style={styles.txt}>{props.text}</Text>
            {
                props.id == 'fingerprint' ?
                <TouchableOpacity style={[styles.right,  {width: 20, backgroundColor: '#fff'}]} onPress={()=>{Alert.alert(props.text)}}>
                    <CheckBox 
                    value={isSelected}
            ...
            }
            {
                ...
                <TouchableOpacity style={styles.right} onPress={()=>{Alert.alert(props.text)}}>
                    <Text style={[styles.number, {marginLeft: 40, color: '#1068FF'}]}>Холбох</Text>
                    <Icon name="chevron-right" solid size={15} style={styles.chevron}/>
                </TouchableOpacity> : null
            }
            {
